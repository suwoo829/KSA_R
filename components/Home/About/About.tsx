    ...
    <div className="pt-16 pb-16 bg-[#dce4e4]">
      ...
      <div className="w-[80%] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-8 items-start mt-20">
        ...
        <div data-aos="fade-left" data-aos-anchor-placement="top-center">
          <h1 className="text-bg text-[32px] sm:text-4xl md:text-5xl lg:text-6xl font-bold text-gray-200">
            {aboutInfo.title}
          </h1>
            ...
            {policyItems.map((policy, index) => (
              <div key={index} className="mb-10">
                <div className="flex items-center space-x-3 mb-3">
                  {/* Increased check icon size */}
                  <div className="w-8 h-8 bg-[#172554] flex flex-col items-center justify-center">
                    <FaCheck className="text-white text-lg" />
                  </div>

                  {/* Increased policy title size */}
                  <p className="text-lg sm:text-xl md:text-2xl font-bold text-black">
                    {policy.title}
                  </p>
                </div>

                {/* Increased policy description size */}
                <p className="ml-11 text-gray-600 text-base md:text-lg">
                  {policy.description}
                </p>
              </div>
            ))}
